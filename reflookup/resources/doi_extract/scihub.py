...
class SciHub:
    ...
    def search(self, query, limit=10, download=False):
        ...
        while True:
            try:
                res = requests.get(SCHOLARS_BASE_URL,
            ...
            if not papers:
                if 'CaptchaRedirect'.encode() in res.content:
                    results[
    ...
    def fetch(self, identifier):
        ...
        try:
            ...
            #           % (identifier, url,
            #              'failed connection' if url else 'captcha')
            #}
    ...
    def _search_direct_url(self, identifier):
        ...
        """
        res = requests.get(SCIHUB_BASE_URL + identifier, headers=HEADERS,
                           verify=False)
        s = self._get_soup(res.content)
